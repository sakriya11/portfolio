...
function Home() {
  ...
  useEffect(() => {
    const handleScroll = () => {
      console.log("scrioolllllllllllllllll", window.scrollY);
  ...
  return (
    <div className="bg-[url('/assets/bg.jpg')] bg-cover overflow-hidden   w-full">
      <div className="flex justify-end items-center h-20 px-20">
        ...
        <button className="lg:relative w-[150px] h-[50px] border border-white rounded-[10px] overflow-hidden group transition-all duration-500 z-[1] "
        onClick={()=>{
          window.open("https://www.linkedin.com/in/sakriya-khadka-74b9432b1/");
        }}
        >
            {/* Left skewed background (replacing ::before) */}
            ...
            {/* Right skewed background (replacing ::after) */}
      ...
      {showScrollIndicator && (
        <div className="flex flex-col items-center absolute bottom-6 left-1/2 -translate-x-1/2 text-white z-50">
      ...
      {/* <button className="group w-[50px] h-[50px] rounded-full bg-[rgb(20,20,20)] border-none font-semibold flex items-center justify-center shadow-[0px_0px_0px_4px_rgba(180,160,255,0.253)] cursor-pointer transition-all duration-300 overflow-hidden relative hover:w-[140px] hover:rounded-[50px] hover:bg-[rgb(181,160,255)]">
        <svg
